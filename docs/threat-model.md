...
| Arbitrary host control | No Docker socket, privileged mode, dynamic mounts, host PID, shell hooks, or broad discovery. Reopen callbacks are loopback-only. |
...
| Malicious archive response | The local adapter treats unexpected existing data as an integrity failure and retains spool work. Future remote adapters must provide equivalent independent verification. |
